...
dict_n_classes = {'clear10':11,
                  'clear100':100}

dict_start_folder = {'clear10':1,
                  'clear100':0}

dict_n_trials = {'clear10':10,
                 'clear100':11}

dict_datapath = {'clear10':'/home/jiawei.dong/projects/CVPR_Challenge_2022/CLEAR/clear10/labeled_images',
                 'clear100':'/home/jiawei.dong/projects/CVPR_Challenge_2022/CLEAR/clear100_cvpr2022/labeled_images'}
...
def get_args_parser():
    parser = argparse.ArgumentParser('CLEAR training script for image classification', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='ecaresnet50t', type=str, metavar='MODEL',
    ...
    parser.add_argument('--lr', type=float, default=4e-3, metavar='LR',
                        help='learning rate (default: 4e-3), with total batch size 64')
    ...
    parser.add_argument('--project', default='clear', type=str,
...
def main(args):
    ...
    for trial in range(args.n_trials):
        if len(args.resume)>0:
            ...
            print('Last trial is %d' % last_trial)
            if trial<last_trial: continue
            
        args.epochs = epochs_trial[trial]
        args.warmup_epochs = min(int(args.epochs * 0.1), 10)
        
        print("Start training for %d epochs" % args.epochs)
        print("Warm up epochs %d" % args.warmup_epochs)
        ...
        if args.disable_eval:
            ...
            dataset_val, _ = build_dataset(is_train=False, args=args, trial=trial)
            print('Length of trial {} eval dataset is {}'.format(trial, len(dataset_val)))
        wandb_logger = None
        if args.epochs == 0:
            continue
            ...
            if (epoch + 1) % args.eval_freq == 0 and data_loader_val is not None:
                ...
                if max_accuracy < test_stats["acc1"]:
                    ...
                    if args.output_dir and args.save_ckpt:
                        utils.save_model(
                            args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                            loss_scaler=loss_scaler, epoch="best", model_ema=model_ema, trial=trial)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('CLEAR training and evaluation script', parents=[get_args_parser()])
    args = parser.parse_args()
    
    args.data_path = dict_datapath[args.clear_dataset]
    args.n_classes = dict_n_classes[args.clear_dataset]
    args.n_trials = dict_n_trials[args.clear_dataset]
    args.start_folder = dict_start_folder[args.clear_dataset]
